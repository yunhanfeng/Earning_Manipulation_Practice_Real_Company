...
df = read.csv('amgen_compustate.csv')
...
# Operating Cash Flow
cash_bf <- data.frame(benford(abs(df$oancf), number.of.digits = 1, 
...
ggplot(data = cash_bf, aes(x = as.factor(Digit), y =Benford_Distribution)) +
  ...
  ggtitle('Theoretical Distribution v.s. Amgen Cash Flow Distribution - 1st Digits ')

# Total Asset
asset_bf <- data.frame(benford(abs(df$at), number.of.digits = 1, 
                               sign = 'positive', discrete = TRUE, round = 3)$bfd)[,c("digits","data.dist", "benford.dist")]
...
ggplot(data = asset_bf, aes(x = as.factor(Digit), y =Benford_Distribution)) +
  ...
  ggtitle('Theoretical Distribution v.s. Amgen TTL Asset Distribution - 1st Digits ')
...
ggplot(data = revenue_bf4, aes(x = Digit, y =Benford_Distribution)) +
  ...
  geom_point(aes(Digit, Sample_Revenue), size = 4, col = 'red')+
  ggtitle('Theoretical Distribution v.s. Amgen Revenue Distribution - 2nd Digits ')
...
ggplot(data = cash_bf4, aes(x = Digit, y =Benford_Distribution)) +
  ...
  ggtitle('Theoretical Distribution v.s. Amgen Cash Flow Distribution - 2nd Digits ')
...
missing_digit <- data.frame('data.digits'=c(2,5), 'count'=c(0,0), 'Percentage' = c(0,0))
at_bf3 <- rbind(at_bf3, missing_digit)
at_bf3 <- at_bf3 %>% arrange(data.digits)
...
ggplot(data = at_bf4, aes(x = Digit, y =Benford_Distribution)) +
  ...
  ggtitle('Theoretical Distribution v.s. Amgen TTL Asset Distribution - 2nd Digits ')
